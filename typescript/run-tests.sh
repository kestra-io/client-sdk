...
log_and_run cd typescript-sdk && npm ci

echo "install SDK locally so it can be imported and used in e2e tests"
log_and_run cd typescript-sdk && npm run build

echo "install test_typescript_sdk dependencies"
log_and_run sh -c 'cd test_typescript_sdk && npm ci'

echo "run test_typescript_sdk tests"
log_and_run sh -c 'cd test_typescript_sdk && npm run test'
