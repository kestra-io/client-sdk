# kestra-api-client — Kestra Java SDK

Official Java SDK for the [Kestra](https://kestra.io) API. Use it to manage
flows, executions and other resources programmatically from a Java
application.
...
- Artifact: [`io.kestra:kestra-api-client`](https://central.sonatype.com/artifact/io.kestra/kestra-api-client)
- Source: <https://github.com/kestra-io/client-sdk> (`java/java-sdk`)
- Requires Java 25+

> This SDK is hand-written. Do not regenerate it — edit the sources under
> `java/java-sdk` directly.
...
To build it from source and install it to your local Maven repository, run
`./gradlew publishToMavenLocal`. Releases are published to Maven Central by CI
(see `.github/workflows/java-sdk-release.yml`).

## Getting started

Build a single `KestraClient` with the builder, then reach every API through its
accessors (`flows()`, `executions()`, `kv()`, ...):
...
public class Example {
    public static void main(String[] args) {
        KestraClient kestraClient = KestraClient.builder()
            ...
            .basicAuth(System.getenv("KESTRA_USERNAME"), System.getenv("KESTRA_PASSWORD"))
        ...
        PagedResultsFlow flows = kestraClient.flows().searchFlows(tenant, 1, 10, null, List.of());
        ...
        String flow = """
            ...
            """;
        FlowWithSource created = kestraClient.flows().createFlow(tenant, flow);
...
API calls throw `io.kestra.sdk.internal.ApiException` (an unchecked exception)
...
try {
    kestraClient.flows().flow("company.team", "does_not_exist", tenant, null, null, null);
} catch (ApiException e) {
    System.err.println("Kestra API returned " + e.getCode() + ": " + e.getResponseBody());
}
```
...
*DashboardsApi* | [**defaultDashboards**](docs/DashboardsApi.md#defaultDashboards) | **GET** /api/v1/{tenant}/dashboards/settings/default-dashboards | Get default dashboards
...
*ExecutionsApi* | [**followDependenciesExecution**](docs/ExecutionsApi.md#followDependenciesExecution) | **GET** /api/v1/{tenant}/executions/{executionId}/follow-dependencies | Follow all execution dependencies executions
...
*KvApi* | [**listKeysWithInheritance**](docs/KvApi.md#listKeysWithInheritance) | **GET** /api/v1/{tenant}/namespaces/{namespace}/kv/inheritance | List all keys for inherited namespaces
