...
> PagedResultsIAMUserControllerApiUserSummary ListUsers(ctx).Filters(filters).Page(page).Size(size).Sort(sort).Execute()
...
func main() {
	...
	configuration := openapiclient.NewConfiguration()
	apiClient := openapiclient.NewAPIClient(configuration)
	resp, r, err := apiClient.UsersAPI.ListUsers(context.Background()).Filters(filters).Page(page).Size(size).Sort(sort).Execute()
	if err != nil {
