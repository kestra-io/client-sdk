...
> IAMInvitationControllerApiInvitationDetail Invitation(ctx, id, tenant).Execute()
...
func main() {
	...
	configuration := openapiclient.NewConfiguration()
	apiClient := openapiclient.NewAPIClient(configuration)
	resp, r, err := apiClient.InvitationsAPI.Invitation(context.Background(), id, tenant).Execute()
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error when calling `InvitationsAPI.Invitation``: %v\n", err)
		fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
	}
	// response from `Invitation`: IAMInvitationControllerApiInvitationDetail
	fmt.Fprintf(os.Stdout, "Response from `InvitationsAPI.Invitation`: %v\n", resp)
}
```
...
Other parameters are passed through a pointer to a apiInvitationRequest struct via the builder pattern
...
> PagedResultsIAMInvitationControllerApiInvitationDetail SearchInvitations(ctx, tenant).Page(page).Size(size).Filters(filters).Sort(sort).Execute()
...
func main() {
	...
	configuration := openapiclient.NewConfiguration()
	apiClient := openapiclient.NewAPIClient(configuration)
	resp, r, err := apiClient.InvitationsAPI.SearchInvitations(context.Background(), tenant).Page(page).Size(size).Filters(filters).Sort(sort).Execute()
	if err != nil {
