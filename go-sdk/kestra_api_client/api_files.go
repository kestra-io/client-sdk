...
// The internal storage uri
func (r ApiFileContentRequest) Path(path string) ApiFileContentRequest {
	r.path = &path
	return r
}

// The revision, if not provided, the latest revision will be returned
func (r ApiFileContentRequest) Revision(revision int32) ApiFileContentRequest {
	r.revision = &revision
	return r
}
...
// The internal storage uri
func (r ApiFileMetadatasRequest) Path(path string) ApiFileMetadatasRequest {
	r.path = &path
	return r
}
...
// The internal storage uri
func (r ApiFileRevisionsRequest) Path(path string) ApiFileRevisionsRequest {
	r.path = &path
	return r
}
...
func (a *FilesAPIService) FileRevisionsExecute(r ApiFileRevisionsRequest) ([]NamespaceFileRevision, *http.Response, error) {
	...
	localVarPath := localBasePath + "/api/v1/{tenant}/namespaces/{namespace}/files/revisions"
	localVarPath = strings.Replace(localVarPath, "{"+"namespace"+"}", url.PathEscape(parameterValueToString(r.namespace, "namespace")), -1)
	localVarPath = strings.Replace(localVarPath, "{"+"tenant"+"}", url.PathEscape(parameterValueToString(r.tenant, "tenant")), -1)
