...
type ApiExportFlowsRequest struct {
	ctx        context.Context
	ApiService *FlowsAPIService
	filters    *[]QueryFilter
	tenant     string
...
func (r ApiExportFlowsRequest) Execute() ([]map[string]interface{}, *http.Response, error) {
	return r.ApiService.ExportFlowsExecute(r)
}

/*
ExportFlows Export all flows as a streamed CSV file

	@param ctx context.Context - for authentication, logging, cancellation, deadlines, tracing, etc. Passed from http.Request or context.Background().
	@param tenant
	@return ApiExportFlowsRequest
*/
func (a *FlowsAPIService) ExportFlows(ctx context.Context, tenant string) ApiExportFlowsRequest {
	return ApiExportFlowsRequest{
		ApiService: a,
		ctx:        ctx,
		tenant:     tenant,
...
func (a *FlowsAPIService) ExportFlowsExecute(r ApiExportFlowsRequest) ([]map[string]interface{}, *http.Response, error) {
	var (
	...
	localBasePath, err := a.client.cfg.ServerURLWithContext(r.ctx, "FlowsAPIService.ExportFlows")
	if err != nil {
		return localVarReturnValue, nil, &GenericOpenAPIError{error: err.Error()}
	}

	localVarPath := localBasePath + "/api/v1/{tenant}/flows/export/by-query/csv"
	localVarPath = strings.Replace(localVarPath, "{"+"tenant"+"}", url.PathEscape(parameterValueToString(r.tenant, "tenant")), -1)
