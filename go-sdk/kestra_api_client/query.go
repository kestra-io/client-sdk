...
//
// Where never panics: a structurally invalid tree (ambiguous leaf+group node or
// nesting deeper than one level) is reported as an error at request time through
// the search / *ByQuery method's existing error return, not at construction.
func Where(root []SearchFilter) []SearchFilter {
	switch {
	case len(root) == 0:
		return []SearchFilter{}
	case len(root) == 1 && root[0].isGroup() && root[0].Logical != nil && *root[0].Logical == LogicalAnd:
