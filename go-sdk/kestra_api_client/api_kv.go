...
func (r ApiListAllKeysRequest) Size(size int32) ApiListAllKeysRequest {
	r.size = &size
	return r
}

// The sort of current page
func (r ApiListAllKeysRequest) Sort(sort []string) ApiListAllKeysRequest {
	r.sort = &sort
	return r
}
...
func (a *KVAPIService) ListAllKeysExecute(r ApiListAllKeysRequest) (*PagedResultsKVEntry, *http.Response, error) {
	...
	localVarPath := localBasePath + "/api/v1/{tenant}/kv"
	localVarPath = strings.Replace(localVarPath, "{"+"tenant"+"}", url.PathEscape(parameterValueToString(r.tenant, "tenant")), -1)
