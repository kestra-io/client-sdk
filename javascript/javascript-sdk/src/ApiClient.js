...
class ApiClient {
	...
	/**
         ...
         */
        this.defaultHeaders = {
            'User-Agent': 'OpenAPI-Generator/0.0.0-dev/Javascript'
        };
        ...
        if (looksLikeQueryFilterArray) {
            ...
            for (const qf of param) {
                ...
                if (typeof qf.value === 'object' && qf.value != null && !Array.isArray(qf.value)) {
                    ...
                    kvpairs[`filters[${keyField}][${op}]`] = this.paramToString(qf.value);
                }
            }
            // IMPORTANT: return a PLAIN OBJECT, not a string.
            return { __kvpairs: kvpairs };
        }

        switch (collectionFormat) {
            case 'csv':   return param.map(this.paramToString, this).join(',');
            case 'ssv':   return param.map(this.paramToString, this).join(' ');
            case 'tsv':   return param.map(this.paramToString, this).join('\t');
            case 'pipes': return param.map(this.paramToString, this).join('|');
            case 'multi': return param.map(this.paramToString, this);
            case 'passthrough': return param;
            default:
    ...
    */
    callApi(path, httpMethod, pathParams,
        ...
        // Flatten any __kvpairs into top-level query params and drop their original keys
        const expandedQuery = {};
        for (const [k, v] of Object.entries(queryParams || {})) {
            if (v && typeof v === 'object' && !Array.isArray(v) && v.__kvpairs) {
                // merge flattened pairs
                Object.assign(expandedQuery, v.__kvpairs);
                // DO NOT keep the original 'filters' key
            } else {
                expandedQuery[k] = v;
            }
        }

        request.query(this.normalizeParams(expandedQuery));
