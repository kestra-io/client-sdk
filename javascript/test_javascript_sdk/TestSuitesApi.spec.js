...
/** @param {string} flowBody */
async function createSimpleFlowFromBody(flowBody) {
    const flow = await kestraClient().flowsApi.createFlow(
        MAIN_TENANT,
        flowBody,
    );
...
/** @param {{namespace: string, id: string}} testSuite */
async function assertTestSuiteExists(testSuite) {
    await expect(
        kestraClient().testSuitesApi.testSuite(
            testSuite.namespace,
            testSuite.id,
            MAIN_TENANT,
        ),
...
async function assertTestSuiteDoesNotExist(testSuite) {
    try {
        await kestraClient().testSuitesApi.testSuite(
            testSuite.namespace,
            testSuite.id,
            MAIN_TENANT,
        );
        ...
        const status =
            e?.status ?? e?.response?.status ?? e?.code ?? e?.data?.code;
...
async function isTestSuiteDisabled(testSuite) {
    const ts = await kestraClient().testSuitesApi.testSuite(
        testSuite.namespace,
        testSuite.id,
        MAIN_TENANT,
    );
...
describe("TestSuitesApiTest", () => {
    it("createTestSuiteTest", async () => {
        ...
        const resp = await kestraClient().testSuitesApi.createTestSuite(
            MAIN_TENANT,
            yaml,
        );
        ...
        expect(resp.description).toBe(
            "assert flow is returning the input value as output",
        );

        const testCaseIds = (resp.testCases ?? []).map(
            (/** @type {{id: string}} */ tc) => tc.id,
        );
    ...
    it("getTestSuiteTest", async () => {
        ...
        const fetched = await kestraClient().testSuitesApi.testSuite(
            namespace,
            testSuiteId,
            MAIN_TENANT,
        );
    ...
    it("deleteTestSuiteTest", async () => {
        ...
        const created = await kestraClient().testSuitesApi.createTestSuite(
            MAIN_TENANT,
            yaml,
        );
        await assertTestSuiteExists(created);

        await kestraClient().testSuitesApi.deleteTestSuite(
            namespace,
            testSuiteId,
            MAIN_TENANT,
        );
    ...
    it("updateTestSuiteTest", async () => {
        ...
        yaml = yaml
            .replace(
                "assert flow is returning the input value as output",
                "updated testsuite description",
            )
            .replace("test_case_1 description", "updated testcase description");

        await kestraClient().testSuitesApi.updateTestSuite(
            namespace,
            testSuiteId,
            MAIN_TENANT,
            yaml,
        ...
        expect((fetched.testCases ?? [])[0]?.description).toBe(
            "updated testcase description",
        );
    });
    ...
    it("validateTestSuiteTest_ok", async () => {
        ...
        const vr = await kestraClient().testSuitesApi.validateTestSuite(
            MAIN_TENANT,
            yaml,
        );
    ...
    it("deleteTestSuiteByIdsTest", async () => {
        const flowId = randomId();
        const namespace = randomId();
        const flow = await createSimpleFlowFromBody(
            LOG_FLOW(flowId, namespace),
        );
        ...
        await kestraClient().testSuitesApi.deleteTestSuitesByIds(MAIN_TENANT, {
            ids: idsToDelete,
        });
    ...
    it("disableTestSuiteByIdsTest", async () => {
        ...
        await kestraClient().testSuitesApi.disableTestSuitesByIds(MAIN_TENANT, {
            ids: idsToDisable,
        });
    ...
    it("enableTestSuiteByIdsTest", async () => {
        ...
        await kestraClient().testSuitesApi.enableTestSuitesByIds(MAIN_TENANT, {
            ids: idsToEnable,
        });
    ...
    it("searchTestSuiteTest", async () => {
        ...
        const flowAAA = await createSimpleFlowFromBody(
            LOG_FLOW(`flowaaa_${randomId()}`, namespaceXXX),
        );
        const flowBBB = await createSimpleFlowFromBody(
            LOG_FLOW(`flowbbb_${randomId()}`, namespaceXXX),
        );
        const flowCCC = await createSimpleFlowFromBody(
            LOG_FLOW(`flowccc_${randomId()}`, namespaceYYY),
        );
    ...
    it("runTestSuiteTest", async () => {
        ...
        const ts = await kestraClient().testSuitesApi.createTestSuite(
            MAIN_TENANT,
            getTestSuiteYaml(SIMPLE_TEST_SUITE, randomId(), namespace, flowId),
        );

        const run = await kestraClient().testSuitesApi.runTestSuite(
            namespace,
            ts.id,
            MAIN_TENANT,
            undefined,
    ...
    it("runTestSuiteByQueryTest", async () => {
        ...
        // by flowId
        {
            ...
            const gotIds = (res?.results ?? [])
                .map((/** @type {{testSuiteId: string}} */ r) => r.testSuiteId)
                .sort();
            expect(gotIds).toEqual([ts1.id, ts2.id].sort());
        }
    ...
    it("searchTestSuitesResultsTest", async () => {
        ...
        // run all of them
        await kestraClient().testSuitesApi.runTestSuite(
            ts1.namespace,
            ts1.id,
            MAIN_TENANT,
            {},
        ...
        // by testSuiteId
        {
            ...
            expect(
                results.every(
                    (/** @type {{testSuiteId: string}} */ r) =>
                        r.testSuiteId === ts1.id,
                ),
            ).toBe(true);
        ...
        // by namespace
        {
            ...
            expect(
                results.every(
                    (/** @type {{namespace: string}} */ r) =>
                        r.namespace === namespaceYYY,
                ),
            ).toBe(true);
            const ids = results
                .map((/** @type {{testSuiteId: string}} */ r) => r.testSuiteId)
    ...
    it("getTestResult", async () => {
        ...
        const fetched = await kestraClient().testSuitesApi.testResult(
            run.id,
            MAIN_TENANT,
        );
    ...
    it("getTestsLastResultTest", async () => {
        ...
        const r1 = await kestraClient().testSuitesApi.runTestSuite(
            namespace,
            ts.id,
            MAIN_TENANT,
            undefined,
        );
        const r2 = await kestraClient().testSuitesApi.runTestSuite(
            namespace,
            ts.id,
            MAIN_TENANT,
            undefined,
        ...
        const fetched = await kestraClient().testSuitesApi.testsLastResult(
            MAIN_TENANT,
            payload,
        );
