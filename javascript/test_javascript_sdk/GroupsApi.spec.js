...
describe("GroupsApi", () => {
    it("add_user_to_group: Add a user to a group", async () => {
        ...
        // add user to group
        const member = await kestraClient().groupsApi.addUserToGroup(
            group.id,
            user.id,
            MAIN_TENANT,
        );
        ...
        expect(
            member.groups?.some(
                /** @param {{id: string}} g */ (g) => g.id === group.id,
            ),
        ).toBeTruthy();
    });
    ...
    it("autocomplete_groups: List groups for autocomplete", async () => {
        ...
        const created = await kestraClient().groupsApi.createGroup(
            MAIN_TENANT,
            {
        ...
        const results = await kestraClient().groupsApi.autocompleteGroups(
            MAIN_TENANT,
            { q: prefix },
        );
        ...
        expect(
            results.some(
                (/** @type {{id: string, name: string}} */ r) =>
                    r.id === created.id || r.name === created.name,
            ),
        ).toBeTruthy();
    });
    ...
    it("get_group: Retrieve a group", async () => {
        ...
        const fetched = await kestraClient().groupsApi.group?.(
            created.id,
            MAIN_TENANT,
        );
    ...
    it("list_group_ids: List groups by ids", async () => {
        ...
        const fetched = await kestraClient().groupsApi.listGroupIds(
            MAIN_TENANT,
            {
                ids: [created.id],
            },
        ...
        expect(
            fetched.some(
                (/** @type {{id: string}} */ g) => g.id === created.id,
            ),
        ).toBeTruthy();
    });
    ...
    it("search_group_members: Search for users in a group", async () => {
        ...
        /** @param {{field: string, operation: 'EQUALS' | 'NOT_EQUALS' | 'GREATER_THAN' | 'LESS_THAN' | 'GREATER_THAN_OR_EQUAL_TO' | 'LESS_THAN_OR_EQUAL_TO' | 'IN' | 'NOT_IN' | 'STARTS_WITH' | 'ENDS_WITH' | 'CONTAINS' | 'REGEX' | 'PREFIX', value: string}} _ */
        ...
        expect(
            results.some(
                /** @param {{id: string}} m */ (m) => m.id === user.id,
            ),
        ).toBeTruthy();
    });
    ...
    it("search_groups: Search for groups", async () => {
        ...
        expect(
            results.some(
                /** @param {{id: string}} r */ (r) => r.id === created.id,
            ),
        ).toBeTruthy();
    });
