...
describe("UsersApi", () => {
    it("autocomplete_users: List users for autocomplete", async () => {
        ...
        await kestraClient().groupsApi.addUserToGroup(
            group.id,
            createdUser.id,
            MAIN_TENANT,
        );
        ...
        const results = await kestraClient().usersApi.autocompleteUsers(
            MAIN_TENANT,
            { q: email },
        );
        ...
        expect(
            results.some(
                (/** @type {{ username: string }} */ r) => r.username === email,
            ),
        ).toBeTruthy();
    });
    ...
    it("create_api_tokens_for_user: Create new API Token for a specific user", async () => {
        ...
        const token = await kestraClient().usersApi.createApiTokensForUser(
            user.id,
            {
    ...
    it("delete_api_token_for_user: Delete an API Token for a user", async () => {
        ...
        const list = await kestraClient().usersApi.listApiTokensForUser(
            user.id,
        );
        const ids =
            list?.results?.map(/** @param {{id:string}} t */ (t) => t.id) ?? [];
    ...
    it("delete_user: Delete a user", async () => {
        ...
        await expect(kestraClient().usersApi.user?.(user.id)).rejects.toThrow();
    });
    ...
    it("delete_user_auth_method: Remove a user auth method", async () => {
        ...
        const patched = await kestraClient().usersApi.deleteUserAuthMethod(
            user.id,
            authId,
        );
    ...
    it("get_user: Retrieve a user", async () => {
        ...
        const fetched = await kestraClient().usersApi.user?.(created.id);
    ...
    it("list_api_tokens_for_user: List API tokens for a user", async () => {
        ...
        const tokens = await kestraClient().usersApi.listApiTokensForUser(
            user.id,
        );
    ...
    it("list_users: Retrieve users", async () => {
        ...
        /**
         *
         * @param {{field: string, operation: any, value: any}} param0
         * @returns
         */
        const qf = ({ field, operation, value }) => ({
            field,
            operation,
            value,
        });
        ...
        expect(
            results.some(
                (/** @type {{id: string}} */ s) => s.id === created.id,
            ),
        ).toBeTruthy();
    ...
    it("patch_user_demo: Update user demo/restricted flag", async () => {
        ...
        await kestraClient().usersApi.patchUserDemo(created.id, {
            restricted: true,
        });
    ...
    it("patch_user_password: Update user password (superadmin op)", async () => {
        ...
        const resp = await kestraClient().usersApi.patchUserPassword(
            created.id,
            {
                password: "NewPass!1",
            },
    ...
    it("patch_user_super_admin: Update user superadmin privileges", async () => {
        ...
        await kestraClient().usersApi.patchUserSuperAdmin(created.id, {
            superAdmin: true,
        });
    ...
    it.skip("update_current_user_password: Change password as the current user", async () => {
        ...
        // const userClient =
        //     /* make a new Kestra client with basic auth (email/initial) */ null;
        ...
        // const userClient2 =
        //     /* make a new Kestra client with basic auth (email/changed) */ null;
