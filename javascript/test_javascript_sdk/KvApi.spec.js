...
describe("KVApi (typed)", () => {
    it("setKeyValue: STRING", async () => {
        ...
        await kestraClient().kvApi.setKeyValue(
            namespace,
            key,
            MAIN_TENANT,
            value,
        ...
        const fetched = await kestraClient().kvApi.keyValue?.(
            namespace,
            key,
            MAIN_TENANT,
        );
    ...
    it("list_keys_with_inheritence: List keys for inherited namespaces", async () => {
        ...
        const entries = await kestraClient().kvApi.listKeysWithInheritence(
            CHILD_NAMESPACE,
            MAIN_TENANT,
        );

        expect(
            entries.some((/** @type {{key: string}} */ e) => e.key === key),
        ).toBeTruthy();
    });
    ...
    it("delete_key_value: Delete a key-value pair", async () => {
        ...
        const deleted = await kestraClient().kvApi.deleteKeyValue(
            CHILD_NAMESPACE,
            key,
            MAIN_TENANT,
        );
    ...
    it("delete_key_values: Bulk-delete multiple key/value pairs", async () => {
        ...
        const resp = await kestraClient().kvApi.deleteKeyValues(
            CHILD_NAMESPACE,
            MAIN_TENANT,
            req,
        );
