...
/**
 * @param {{namespace: string, id: string}} flow
 */
...
describe("FlowsApi", () => {
    ...
    it("bulk_update_flows: Update from multiple yaml sources", async () => {
        ...
        const updatedBody = flowBody.replace(
            "simple_flow_description",
    ...
    it("delete_flow", async () => {
        ...
        await kestraClient().flowsApi.deleteFlow(
            flow.namespace,
            flow.id,
            MAIN_TENANT,
        );
    ...
    it("delete_flows_by_ids", async () => {
        ...
        await kestraClient().flowsApi.deleteFlowsByIds(
            MAIN_TENANT,
            idWithNamespace,
        );
    ...
    it("disable_flows_by_ids", async () => {
        const flow = await createSimpleFlow();
        const idWithNamespace = [{ id: flow.id, namespace: flow.namespace }];
        await kestraClient().flowsApi.disableFlowsByIds(
            MAIN_TENANT,
            idWithNamespace,
        );
    });
    ...
    it("enable_flows_by_ids", async () => {
        const flow = await createSimpleFlow();
        const idWithNamespace = [{ id: flow.id, namespace: flow.namespace }];
        await kestraClient().flowsApi.enableFlowsByIds(
            MAIN_TENANT,
            idWithNamespace,
        );
    });
    ...
    it("export_flows_by_ids", async () => {
        ...
        await kestraClient().flowsApi.exportFlowsByIds(
            MAIN_TENANT,
            idWithNamespace,
        );
    });
    ...
    it("generate_flow_graph", async () => {
        ...
        const subflows = undefined;
        await kestraClient().flowsApi.generateFlowGraph(
            namespace,
            id,
            MAIN_TENANT,
            { revision, subflows },
    ...
    it("generate_flow_graph_from_source", async () => {
        const body = getSimpleFlow();
        const subflows = undefined;
        await kestraClient().flowsApi.generateFlowGraphFromSource(
            MAIN_TENANT,
            body,
            subflows,
        );
    ...
    it("get_task_from_flow", async () => {
        ...
        await kestraClient().flowsApi.taskFromFlow(
            namespace,
            id,
            taskId,
            MAIN_TENANT,
    ...
    it("list_distinct_namespaces", async () => {
        ...
        const q = undefined;
        const resp = await kestraClient().flowsApi.listDistinctNamespaces(
            MAIN_TENANT,
            q,
        );
    ...
    it("list_flows_by_namespace", async () => {
        const flow = await createSimpleFlow();
        const { namespace } = flow;
        const resp = await kestraClient().flowsApi.listFlowsByNamespace(
            namespace,
            MAIN_TENANT,
        );
        expect(Array.isArray(resp)).toBe(true);
    ...
    it("update_flow", async () => {
        ...
        const resp = await kestraClient().flowsApi.updateFlow(
            namespace,
            id,
            MAIN_TENANT,
            updatedBody,
    ...
    it("validate_task", async () => {
        ...
        const resp = await kestraClient().flowsApi.validateTask(
            section,
            MAIN_TENANT,
            taskObj,
        );
    ...
    it("validate_task_invalid", async () => {
        ...
        expect(constraints.join(" ")).toMatch(
            /Invalid type: io\.kestra\.plugin\.core\.log\.InvalidTask/,
        );
    });
    ...
    it("validate_trigger", async () => {
        ...
        const resp = await kestraClient().flowsApi.validateTrigger(
            MAIN_TENANT,
            triggerObj,
        );
        expect(resp.constraints ?? []).toHaveLength(0);
    ...
    it("validate_trigger_invalid", async () => {
        ...
        expect(constraints.join(" ")).toMatch(
            /Invalid type: io\.kestra\.plugin\.core\.trigger\.InvalidType/,
        );
    });
});
