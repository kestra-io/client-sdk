...
describe("NamespacesApi", () => {
    it("autocomplete_namespaces: List namespaces for autocomplete", async () => {
        ...
        const ns = await kestraClient().namespacesApi.createNamespace(
            MAIN_TENANT,
            {
                id: prefix,
                deleted: false,
        ...
        const results =
            await kestraClient().namespacesApi.autocompleteNamespaces(
                MAIN_TENANT,
                { q: prefix },
            );

        expect(
            results.some((/** @type {string} */ r) => r === ns.id),
        ).toBeTruthy();
    });

    it("create_namespace: Create a namespace", async () => {
        const nsId = `test_create_namespace_${randomId()}`;
        const created = await kestraClient().namespacesApi.createNamespace(
            MAIN_TENANT,
            {
                id: nsId,
                deleted: false,
    ...
    it("delete_namespace: Delete a namespace", async () => {
        ...
        await kestraClient().namespacesApi.deleteNamespace(
            created.id,
            MAIN_TENANT,
        );
    ...
    it("get_inherited_secrets: List inherited secrets", async () => {
        ...
        const inherited = await kestraClient().namespacesApi.inheritedSecrets(
            ns.id,
            MAIN_TENANT,
        );
    ...
    it("get_namespace: Get a namespace", async () => {
        ...
        const fetched = await kestraClient().namespacesApi.namespace?.(
            created.id,
            MAIN_TENANT,
        );
    ...
    it("inherited_plugin_defaults: List inherited plugin defaults", async () => {
        ...
        const defaults =
            await kestraClient().namespacesApi.inheritedPluginDefaults(
                ns.id,
                MAIN_TENANT,
            );
    ...
    it("inherited_variables: List inherited variables", async () => {
        ...
        const variables = await kestraClient().namespacesApi.inheritedVariables(
            ns.id,
            MAIN_TENANT,
        );
    ...
    it("list_namespace_secrets: Get secrets for a namespace (with filter)", async () => {
        ...
        await kestraClient().namespacesApi.putSecrets(ns.id, MAIN_TENANT, {
            key,
            value: "list-value",
        });
        await kestraClient().namespacesApi.putSecrets(ns.id, MAIN_TENANT, {
        ...
        const gotKeys = results.map((/** @type {{key: string}} */ m) => m.key);
    ...
    it("patch_secret: Patch secret metadata for a namespace", async () => {
        ...
        const metas = await kestraClient().namespacesApi.patchSecret(
            ns.id,
            key,
            MAIN_TENANT,
            meta,
    ...
    it("search_namespaces: Search for namespaces", async () => {
        ...
        expect(
            results.some(/** @param {{id: string}} r */ (r) => r.id === ns.id),
        ).toBeTruthy();
    });
    ...
    it("delete_secret: Delete a secret for a namespace", async () => {
        ...
        await kestraClient().namespacesApi.deleteSecret(
            ns.id,
            key,
            MAIN_TENANT,
        );
        ...
        expect(
            results.some(/** @param {{key: string}} m */ (m) => m.key === key),
        ).toBeFalsy();
    });
});
