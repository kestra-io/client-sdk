...
describe("ServiceAccountApi", () => {
    it("create_service_account", async () => {
        ...
        const created =
            await kestraClient().serviceAccountApi.createServiceAccount({
                name,
    ...
    it("create_service_account_for_tenant", async () => {
        ...
        const created =
            await kestraClient().serviceAccountApi.createServiceAccountForTenant(
                MAIN_TENANT,
                {
                    name,
    ...
    it("delete_service_account_for_tenant", async () => {
        ...
        await kestraClient().serviceAccountApi.deleteServiceAccountForTenant(
            created.id,
            MAIN_TENANT,
        );
    });
    ...
    it("get_service_account", async () => {
        ...
        const fetched = await kestraClient().serviceAccountApi.serviceAccount(
            created.id,
        );
    ...
    it("get_service_account_for_tenant", async () => {
        ...
        const fetched =
            await kestraClient().serviceAccountApi.serviceAccountForTenant(
                created.id,
                MAIN_TENANT,
            );
    ...
    it("list_service_accounts (superadmin only)", async () => {
        ...
        const results =
            await kestraClient().serviceAccountApi.listServiceAccounts(
                1,
                10000,
                [],
            );
        ...
        const items = Array.isArray(results)
            ? results
            : (results?.results ?? []);
        const ids = items.map((/** @type {{id: string}} */ r) => r?.id);
    ...
    it("patch_service_account_details", async () => {
        ...
        const patched =
            await kestraClient().serviceAccountApi.patchServiceAccountDetails(
                created.id,
                {
                    name,
                    description: "new",
    ...
    it("patch_service_account_super_admin", async () => {
        ...
        await kestraClient().serviceAccountApi.patchServiceAccountSuperAdmin(
            created.id,
            { superAdmin: true },
        );
    ...
    it("update_service_account", async () => {
        ...
        const updated =
            await kestraClient().serviceAccountApi.updateServiceAccount(
                created.id,
                MAIN_TENANT,
                {
