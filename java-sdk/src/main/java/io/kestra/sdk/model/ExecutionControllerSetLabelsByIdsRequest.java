...
public class ExecutionControllerSetLabelsByIdsRequest {
  ...
  public String toUrlQueryString(String prefix) {
    ...
    // add `executionsId` to the URL query string
    if (getExecutionsId() != null) {
      for (int i = 0; i < getExecutionsId().size(); i++) {
        try {
          joiner.add(String.format("%sexecutionsId%s%s=%s", prefix, suffix,
              "".equals(suffix) ? "" : String.format("%s%d%s", containerPrefix, i, containerSuffix),
              URLEncoder.encode(String.valueOf(getExecutionsId().get(i)), "UTF-8").replaceAll("\\+", "%20")));
    ...
    // add `executionLabels` to the URL query string
    if (getExecutionLabels() != null) {
      for (int i = 0; i < getExecutionLabels().size(); i++) {
        if (getExecutionLabels().get(i) != null) {
          joiner.add(getExecutionLabels().get(i).toUrlQueryString(String.format("%sexecutionLabels%s%s", prefix, suffix,
              "".equals(suffix) ? "" : String.format("%s%d%s", containerPrefix, i, containerSuffix))));
        }
      }
    }
