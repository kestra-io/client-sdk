...
public class AppsControllerApiBulkOperationRequest {
  ...
  public String toUrlQueryString(String prefix) {
    ...
    // add `uids` to the URL query string
    if (getUids() != null) {
      for (int i = 0; i < getUids().size(); i++) {
        try {
          joiner.add(String.format("%suids%s%s=%s", prefix, suffix,
              "".equals(suffix) ? "" : String.format("%s%d%s", containerPrefix, i, containerSuffix),
              URLEncoder.encode(String.valueOf(getUids().get(i)), "UTF-8").replaceAll("\\+", "%20")));
