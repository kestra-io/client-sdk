...
public class IAMRoleControllerApiRoleCreateOrUpdateRequestPermissions {
  ...
  public String toUrlQueryString(String prefix) {
    ...
    // add `TENANT_ACCESS` to the URL query string
    if (getTENANTACCESS() != null) {
      for (int i = 0; i < getTENANTACCESS().size(); i++) {
        try {
    ...
    if (getSERVICEACCOUNT() != null) {
      for (int i = 0; i < getSERVICEACCOUNT().size(); i++) {
        try {
          joiner.add(String.format("%sSERVICE_ACCOUNT%s%s=%s", prefix, suffix,
              "".equals(suffix) ? "" : String.format("%s%d%s", containerPrefix, i, containerSuffix),
              URLEncoder.encode(String.valueOf(getSERVICEACCOUNT().get(i)), "UTF-8").replaceAll("\\+", "%20")));
        } catch (UnsupportedEncodingException e) {
          // Should never happen, UTF-8 is always supported
          throw new RuntimeException(e);
        }
    ...
    // add `INVITATION` to the URL query string
    if (getINVITATION() != null) {
      for (int i = 0; i < getINVITATION().size(); i++) {
        try {
          joiner.add(String.format("%sINVITATION%s%s=%s", prefix, suffix,
              "".equals(suffix) ? "" : String.format("%s%d%s", containerPrefix, i, containerSuffix),
              URLEncoder.encode(String.valueOf(getINVITATION().get(i)), "UTF-8").replaceAll("\\+", "%20")));
        } catch (UnsupportedEncodingException e) {
          // Should never happen, UTF-8 is always supported
          throw new RuntimeException(e);
        }
    ...
    // add `GROUP_MEMBERSHIP` to the URL query string
    if (getGROUPMEMBERSHIP() != null) {
      for (int i = 0; i < getGROUPMEMBERSHIP().size(); i++) {
        try {
          joiner.add(String.format("%sGROUP_MEMBERSHIP%s%s=%s", prefix, suffix,
              "".equals(suffix) ? "" : String.format("%s%d%s", containerPrefix, i, containerSuffix),
              URLEncoder.encode(String.valueOf(getGROUPMEMBERSHIP().get(i)), "UTF-8").replaceAll("\\+", "%20")));
        } catch (UnsupportedEncodingException e) {
          // Should never happen, UTF-8 is always supported
          throw new RuntimeException(e);
        }
