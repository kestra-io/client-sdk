...
class BindingsApi(BaseApi):
    """IAM bindings (`/api/v1/{tenant}/bindings`).

    A binding grants a role to a user or group, optionally scoped to a namespace.
    """

    def create_binding(
        self, tenant: str, request: IAMBindingControllerApiCreateBindingRequest
    ) -> IAMBindingControllerApiBindingDetail:
        path = self._tenant_path(tenant, "bindings")
        return self._json_request(
            "POST", path, IAMBindingControllerApiBindingDetail, body=request
        )

    def bulk_create_binding(
        self, tenant: str, requests: List[IAMBindingControllerApiCreateBindingRequest]
    ) -> List[IAMBindingControllerApiBindingDetail]:
        path = self._tenant_path(tenant, "bindings", "bulk")
        return self._json_list_request(
            "POST", path, IAMBindingControllerApiBindingDetail, body=requests
        )
    ...
    def search_bindings(
        ...
        params = list(
            self._build_query_params(
                page=page, size=size, id=external_id, namespace=namespace
            ).items()
        )
        if binding_type is not None:
            params.append(("type", binding_type.value))
        self._append_repeated_param(params, "sort", sort)
        self._append_filter_params(params, filters)
        return self._json_request(
            "GET", path, PagedResultsIAMBindingControllerApiBindingDetail, params=params
        )
