...
# **search_roles**
> PagedResultsApiRoleSummary search_roles(tenant, q=q, page=page, size=size, sort=sort)
...
with KestraClient(configuration) as kestra_client:
    ...
    try:
        # Search for roles
        api_response = kestra_client.RolesApi.search_roles(tenant, q=q, page=page, size=size, sort=sort)
